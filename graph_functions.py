...
class PoseEstimator:
    ...
    def __init__(self, image_size):
        ...
        with tf.variable_scope('CPM'):
            ...
            self.image_in = tf.placeholder(tf.float32, [1, utils.config.INPUT_SIZE, self.img_size[1], 3])
            heatmap_person = utils.inference_person(self.image_in)
            self.heatmap_person_large = tf.image.resize_images(heatmap_person,
                                                               [utils.config.INPUT_SIZE, self.img_size[1]])

            # placeholders for pose network
            self.pose_image_in = tf.placeholder(tf.float32,
                                                [_N, utils.config.INPUT_SIZE, utils.config.INPUT_SIZE, 3])
            self.pose_centermap_in = tf.placeholder(tf.float32,
                                                    [_N, utils.config.INPUT_SIZE, utils.config.INPUT_SIZE, 1])
            self.heatmap_pose = utils.inference_pose(self.pose_image_in, self.pose_centermap_in)

    def load_model(self, sess_path):
    ...
    def estimate(self, image, sess):
        """Estimate 2d and 3d poses on the image.
        INPUT:
            image: RGB image in the format (w x h x 3)
            sess: tensorflow session
        OUTPUT:
            pose_2d: 2D pose for each of the people in the image in the format (num_ppl x num_joints x 2)
            visibility: vector containing a bool value for each joint representing the visibility of the
                        joint in the image (could be due to occlusions or the joint is not in the image)
            pose_3d: 3D pose for each of the people in the image in the format (num_ppl x 3 x num_joints)"""
        ...
        hmap_person = sess.run(self.heatmap_person_large, {self.image_in: b_image})

        hmap_person = np.squeeze(hmap_person)
        centers = utils.detect_objects_heatmap(hmap_person)
        b_pose_image, b_pose_cmap = utils.prepare_input_posenet(b_image[0], centers,
        ...
        # Estimate 2D poses
        estimated_2d_pose, visibility = utils.detect_parts_heatmaps(_hmap_pose, centers,
                                                         [utils.config.INPUT_SIZE, utils.config.INPUT_SIZE])
        # Estimate 3D poses
        transformed_pose2d, weights = self.poseLifting.transform_joints(estimated_2d_pose.copy(), visibility)
